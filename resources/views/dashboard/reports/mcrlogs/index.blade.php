            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                var calendar = new FullCalendar.Calendar(ProdCal, {
                    ...
                    eventClick: function(info) {
                        window.location = "/logs/mcr/" + info.event.id + "/edit";

                    },
                    eventMouseEnter: function(info) {
                        console.log('eventMouseEnter');
            var tis=info.el;
            var tooltip = '<div class="tooltipevent" style="top:'+($(tis).offset().top-5)+'px;left:'+($(tis).offset().left+($(tis).width())/2)+'px"><div>' + info.event.title + '</div><div>' + info.event.extendedProps.remarks + '</div></div>';
