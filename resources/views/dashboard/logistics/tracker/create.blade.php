        ...
        <div class="row">
            <div class="col-12">
                <div class="card-opaque">
                    ...
                    <div class="card-body">
                        <form method="POST" enctype="multipart/form-data" action="{{ route('tracker.store',) }}">
                            @csrf
                            ...
                            <div class="row justify-content-between">
                                ...
                                @if(count($mileage_trackers) > 1 )
                                @php
                                    $id = \DB::table('trackers')->latest('id')->first();
                                     $id = $id->id;
                                    $last_odometer_reading = \App\Models\Tracker::where('id', $id)->orderBy('id','desc')->max('odometer_reading');
                                @endphp
                                @elseif ((count($mileage_trackers) === 1 ))
                                @php
                                    $id = \DB::table('trackers')->latest('id')->first();
                                     $id = $id->id;
                                    $last_odometer_reading  = \App\Models\Tracker::where('id', $id)->orderBy('id','desc')->max('odometer_reading');
                                ...
                                <div class=" row justify-content-between">
                                    <div class="mb-3 col-md-6">
                                        ...
                                        <input name="odometer_reading" type="text" class="form-control"
                                            id="odometer_reading" value="{{ old('last_odometer_reading') }}" required
                                            placeholder="">
                                    </div>
                                    <input name="last_odometer_reading" type="text" class="form-control"
                                    id="last_odometer_reading" value="{{ $last_odometer_reading }}" required hidden
                                    placeholder="">
                                    <div class="mb-3 col-md-6">
                                        <label class="form-label">Refuel Date</label>
                                        <div class="input-group date" id="datetimepicker-minimum"
                                            data-target-input="nearest">
                                            <input name="refuel_date" type="text" class="form-control datetimepicker-input"
